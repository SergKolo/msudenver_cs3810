...
Ineritance of files and filetype is futher illustrated in the diagram below. To facilitate queries another entity default_application will be used.
...
The application is written as command-line application, which implies the application can interface with the user via text. The user can control which database queries are issued via command-line switches such as `-l`, `-u`, `-v`. For this reason a proper terminal emulator or default Linux virtual console is required.
